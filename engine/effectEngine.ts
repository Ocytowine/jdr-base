...
import { pushProficiencyEntry, normalizeProficiencySummary, type ProficiencySummary } from '~/utils/proficiencies'
...
export class EffectEngine {
  ...
  async applyEffect(character: Character, effect: Effect, ctx: EvalContext = {}) {
    ...
    if (
      ...
      character.proficiency_summary = normalizeProficiencySummary(character.proficiency_summary ?? {});
      const summary = character.proficiency_summary as ProficiencySummary;
      const effectSource = effect.source ?? effect.raw?.source ?? null;

      const registerNormalized = (normalized: ProficiencySummary) => {
        for (const [cat, entries] of Object.entries(normalized || {})) {
          for (const entry of entries) {
            const registered = pushProficiencyEntry(summary, entry, { categoryHint: cat, source: effectSource ?? null });
            const catKey = String(cat || '').toLowerCase();
            if (
              registered &&
              catKey === 'competences' &&
              !character.proficiencies.includes(registered.id)
            ) {
              character.proficiencies.push(registered.id);
            }
          }
        }
      };

      const absorb = (categoryHint: string | null | undefined, value: any) => {
        if (value === null || value === undefined) return;
        const normalized =
          categoryHint !== null && categoryHint !== undefined
            ? normalizeProficiencySummary({ [categoryHint]: value })
            : normalizeProficiencySummary(value);
        registerNormalized(normalized);
      };

      const structuredMappings: Array<{ keys: string[]; category: string }> = [
      ...
      for (const mapping of structuredMappings) {
        for (const key of mapping.keys) {
          if (Object.prototype.hasOwnProperty.call(payload, key)) {
            absorb(mapping.category, (payload as any)[key]);
          }
        }
      }

      absorb(null, payload.proficiency_grant);
      absorb(null, payload.categories);
      absorb('skills', payload.skill);
      absorb('skills', payload.skills);
      absorb(null, payload.proficiencies);
      absorb(null, payload.proficiency);
      absorb(null, payload.value);
      ...
      if (payload.subtype) {
        const hintedCategory =
          typeof payload.category === 'string'
            ...
            ? payload.type
            : 'weapons';
        absorb(hintedCategory, payload.subtype);
      }

      if (payload.proficiency_id) {
        absorb(typeof payload.category === 'string' ? payload.category : undefined, payload.proficiency_id);
      }
    ...
    if (
      ...
      for (const entry of toArray(candidates)) {
        ...
        pushProficiencyEntry(summary, normalized, { categoryHint: 'saving_throws', source: effectSource ?? null });
      }
      return;
    }
